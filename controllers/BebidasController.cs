...
public class BebidasController(AppDbContext db) : ControllerBase
{
    [HttpGet]
    public async Task<ActionResult<object>> GetAll([FromQuery] bool? ativo)
    {
        var q = db.Bebidas.AsNoTracking().AsQueryable();
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<BebidaReadDto>> GetById(int id)
    {
        var e = await db.Bebidas.AsNoTracking().FirstOrDefaultAsync(x => x.Id == id);
        return e is null ? NotFound() : Ok(e.ToReadDto());
    }

    [HttpPost]
    public async Task<ActionResult<BebidaReadDto>> Create(BebidaCreateDto dto)
    {
        var e = dto.ToEntity();
        db.Bebidas.Add(e);
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, BebidaUpdateDto dto)
    {
        if (id != dto.Id) return BadRequest();
        var e = await db.Bebidas.FirstOrDefaultAsync(x => x.Id == id);
    ...
    {
        var e = await db.Bebidas.FindAsync(id);
        if (e is null) return NotFound();
        e.Ativo = ativo;
        await db.SaveChangesAsync();
        return NoContent();
